...
def victory(s):
    for r in range(rows):
        for c in range(columns):
            if not s.board[r][c] == 0:
                count = 0
                for k in range(0, 4):
                    if (c + k) < columns:
                        if s.board[r][c + k] == HUMAN:
                            count = count + 1
                        else:
                            break
                        if count == 4:
                ...
                count = 0
                for k in range(0, 4):
                    if rows > (r + k) and columns > (c + k):
                        if s.board[r + k][c + k] == HUMAN:
                            count = count + 1
                        else:
                            break
                        if count == 4:
    ...
    for r in range(rows):
        for c in range(columns):
            if not s.board[r][c] == 0:
                ...
                for k in range(0, 5):
                    if (r + k) < rows and c < columns:
                        if s.board[r + k][c] == COMPUTER:
                            count = count + 1
                        else:
                            break
                        if count == 4:
